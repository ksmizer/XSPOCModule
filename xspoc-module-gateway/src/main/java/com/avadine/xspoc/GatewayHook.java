...
import com.avadine.xspoc.collector.CollectorConfiguration;
import com.avadine.xspoc.collector.CollectorConfigurationPage;
import com.avadine.xspoc.collector.CollectorDatabaseConnection;
import com.avadine.xspoc.collector.CollectorRunnable;
import com.avadine.xspoc.service.ScriptFunctions;
...
public class GatewayHook extends AbstractGatewayModuleHook {
    ...
    public void startup(LicenseState licenseState) {
        ...
        executionManager = context.createExecutionManager("xspoc Collector", threads);
        ...
        for (Integer i = 0; i < threads; i++) {
            if (i < collectionSourceIds.size()){
                usedThreads.add(i);
                executionManager.registerAtFixedRate("xspoc", "Collector - Thread: " + Integer.toString(i+1), new CollectorRunnable(context, collectionSourceIds.get(i)), 60, TimeUnit.SECONDS);
            }
        }
    }
    ...
    public void shutdown() {
        for (Integer i : usedThreads) {
            executionManager.unRegister("xspoc", "Collector - Thread: " + Integer.toString(i+1));
        }
